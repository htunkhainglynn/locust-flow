    ...
    def on_start(self):
        ...
        try:
            ...
            if run_init_once:
                with self.__class__._init_lock:
                    if not self.__class__._init_completed:
                        ...
                        if user_list_var:
                            ...
                            if not user_list:
                                raise ValueError(f"No list found in variables['{user_list_var}'] for run_init_once mode")
                            
                            logging.info(f"Initializing {len(user_list)} users from '{user_list_var}': {user_list}")
                            ...
                            shared_flow_executor.context['_data_store'] = self.__class__._data_store
                        ...
                        self.__class__._shared_context = user_flow_executor.get_context()
                        ...
                        if self.__class__._data_store.get_count() > 0:
                            logging.info(f"All initialized users: {self.__class__._data_store.get_all_identifiers()}")
                ...
                logging.info(f"User initialized with shared context for service: {self.config.get('service_name')}")
            else:
                self.flow_executor = FlowExecutor(self.config)
                self.flow_executor.context['_data_store'] = self.__class__._data_store
                init_steps = self.config.get('init', [])
...
def create_user_class(config_file_path: str, wait_time=None, class_name=None):
    ...
    if not class_name:
        try:
            ...
            class_name = f"ConfigUser_{config_file_path.replace('.', '_').replace('/', '_')}"
