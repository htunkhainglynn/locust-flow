...
class ConfigValidator:
    ...
    def validate(self, config: Dict[str, Any], config_file: str = "config") -> Tuple[bool, List[str], List[str]]:
        """
    ...
    def _validate_top_level_keys(self, config: Dict[str, Any]):
        ...
        valid_top_level_keys = [
            'service_name', 'base_url', 'variables',
            'init', 'steps', 'cleanup',
            'run_init_once', 'init_list_var',
    ...
    def _validate_step(self, step: Dict[str, Any], path: str):
        ...
        valid_step_keys = [
            'name', 'method', 'endpoint', 'headers', 'data', 'params', 'json',
            'pre_request', 'pre_transforms', 'post_transforms',
            'extract', 'validate', 'retry_on', 'skip_if', 'weight',
            'timeout', 'allow_redirects', 'verify', 'cert', 'auth'
        ]
    ...
    def _validate_variables(self, config: Dict[str, Any]):
        ...
        if not variables:
            self.warnings.append("No 'variables' defined. Consider adding reusable variables.")
            return
    ...
    def _validate_validation_step(self, validate: Any, path: str):
        ...
        if isinstance(validate, dict):
            ...
            for idx, item in enumerate(validate):
                ...
                old_format_keys = {'status_code', 'max_response_time', 'json', 'fail_on_error'}
                ...
                if has_field_based:
                    ...
                    for field in required:
                        if field not in item:
                            self.errors.append(f"{path}[{idx}]: Missing required field '{field}'")
    ...
    def _validate_transforms(self, config: Dict[str, Any]):
        ...
        valid_types = [
            'rsa_encrypt', 'hmac', 'sha256', 'base64_encode', 'base64_decode',
            'uuid', 'timestamp', 'random_number', 'random_choice', 'random_string',
            'increment', 'select_from_list', 'select_msisdn', 'store_data'
        ]
    ...
    def _validate_transform_list(self, transforms: Any, path: str, valid_types: list, valid_modes: list, variables: Dict[str, Any] = None):
        ...
        for idx, transform in enumerate(transforms):
            ...
            if transform_type == 'select_from_list':
                self._validate_select_from_list_config(transform, f"{path}[{idx}]", valid_modes, variables)
    ...
    def _validate_select_from_list_config(self, transform: Dict[str, Any], path: str, valid_modes: list, variables: Dict[str, Any] = None):
        ...
        if 'from' not in config:
            self.errors.append(f"{path}.config: Missing required field 'from' (variable name)")
        else:
